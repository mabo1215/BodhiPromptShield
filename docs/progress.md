...
- 外部 transfer 与更强 external validation 仍未完成；修改说明：当前论文仍主要基于 CPPB 与 controlled evaluation，尚未新增 public benchmark transfer 或 truly independent external validation，但论文内已把 protocol-compatible benchmark 的优先顺序明确收口为 TAB、2014 i2b2/UTHealth、SROIE/CORD、FUNSD 与 AgentDojo；未修改原因：这一轮补齐的是具名 benchmark 路线与协议映射，而不是伪造尚未运行的外部实验；后续准备如何修改：按附录中的优先顺序先做 TAB / i2b2 text transfer，再做 SROIE/CORD/FUNSD 的 multimodal 与 hard-case supporting slice。
- 可复现性元数据仍有剩余缺口；修改说明：本轮已把 remaining metadata gaps 正式写入 `paper/appendix.tex`，并在 `paper/main.tex` 中同步解释这些 manifest 应如何界定当前证据边界，但 cross-model 结果仍缺 exact backbone identifiers / config logs，multimodal slice 仍缺 exact OCR engine/version manifest，latency 仍缺 hardware-specific host manifest；未全部修改原因：这些是重要但不改变主要结论的 metadata completeness 问题，且当前公开快照并不包含足够原始运行日志去诚实恢复全部精确标识；后续准备如何修改：在下一轮把剩余 metadata 缺口继续整理成独立 manifest 或完整 rerun artifact。
- CPPB data-card 仍可继续补强；修改说明：当前 prompt-level inventory、distribution breakdown、accounting summary、artifact metadata note 以及论文内新增的 release/data-card prose 已进一步收口到 source-level provenance、annotation examples、sanitization-policy labels 与 external wrapper specification 这些 benchmark-card 核心字段，但更完整 multimodal provenance、逐源 licensing package 与示例级原始资产说明仍不完整；未全部修改原因：本轮优先把与 reproducibility scope 最直接相关的 data-card 字段写清，而不是一次性扩成完整 benchmark package；后续准备如何修改：继续补 fuller data-card documentation，并把外部 benchmark wrapper 与 multimodal provenance 一并并入 appendix 的可复现性边界说明。
