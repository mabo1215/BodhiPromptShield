...
- 更强的 external practical baseline family 仍未全部补齐；修改说明：当前已补 Presidio 风格基线，但 prompted LLM zero-shot de-identification、更多 enterprise-grade 或更通用的 external pipeline 仍未做 matched benchmark；未全部修改原因：本轮优先完成 threat model 对应实验和 multi-seed / leave-template-out 两项缺口；后续准备如何修改：仅在保持同一 CPPB protocol 和相同 evaluation slice 的前提下再补更强外部基线。
...
- 可复现性元数据仍有剩余缺口；修改说明：cross-model 结果仍缺 exact backbone identifiers / config logs，multimodal slice 仍缺 OCR engine/version manifest 与更完整 source metadata，latency table 仍缺 hardware / concurrency manifest；未全部修改原因：这些是重要但不改变主要结论的 metadata completeness 问题；后续准备如何修改：在下一轮把剩余 metadata 缺口整理成独立 manifest 或 appendix note。
- CPPB data-card 仍可继续补强；修改说明：当前 prompt-level inventory、distribution breakdown 与 accounting summary 已经补齐，但 annotation guide、split metadata、source-level release metadata 仍不完整；未全部修改原因：本轮优先补 exact counts 和 benchmark card，而不是把所有 data-card 细节一次写满；后续准备如何修改：继续补 fuller data-card documentation，并与 appendix 中的 reproducibility scope 对齐。
